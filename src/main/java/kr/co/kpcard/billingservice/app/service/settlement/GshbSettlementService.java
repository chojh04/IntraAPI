...
@Service
public class GshbSettlementService {
	
	private Logger logger = LoggerFactory.getLogger(GshbSettlementService.class);
	...
	@Value("${http.gshb.id}")
	String httpId;	
	@Value("${http.gshb.upload.name}")
	String uploadName;
	@Value("${http.gshb.download.name}")
	String downloadName;
	@Value("${http.gshb.otisOnline}")
	String otisOnline;
	@Value("${http.gshb.pgOnline}")
	String pgOnline;
	
	private final String JOB_DIVIDER = JobStatusCode.GSHB;
		...
		try
		{
			...
			if(settleResultUnmatchedList.size()>0)
			{
				for(int idx=0; idx<loopCnt; idx++)
				{
					...
					logger.info("loopcnt:"+loopCnt+"/fromIndex:"+fromIndex+"/toIndex"+toIndex);
			...
			//settlementJobHistoryService.jobInsertSettleResultCount(JOB_DIVIDER, settleDate);
			logger.info("★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★");
