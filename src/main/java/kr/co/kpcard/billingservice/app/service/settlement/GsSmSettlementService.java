...
@Service
public class GsSmSettlementService {
	
	private Logger logger = LoggerFactory.getLogger(GsSmSettlementService.class);
	...
	@Value("${ftp.gsSm.upload.name}")
	String uploadName;
	@Value("${ftp.gsSm.download.name}")
	String downloadName;
	
	@Value("${ftp.gsSm.otisOnline}")
	String otisOnline;
	@Value("${ftp.gsSm.pgOnline}")
	String pgOnline;
	
	private final String JOB_DIVIDER = JobStatusCode.GSSM;
	...
	public boolean gsSettlementByDuration(Date startDate, Date endDate)
	{
		...
		{
			downloadDate = DateUtil.addDay(startDate, addDay);
			result = gsDailySettlement(DateUtil.getYear(downloadDate), DateUtil.getMonth(downloadDate), DateUtil.getDay(downloadDate));
		}while((addDay++) < diffDays);
		
		return result;
	}	
	
	public boolean gsDailySettlement(int year, int month, int day) 
	{
	...
	public boolean settlement(int year, int month, int day)
	{
		...
		try
		{
			...
			List<GsSettlementSelect> resultList = gsSettlementMapper.settlementSelectGssm(JOB_DIVIDER, otisOnline, settleDate, pgOnline);
	...
	public boolean download(int year, int month, int day)
	{
		...
		try 
		{
			...
			resultList = apacheFtpManager.getFileDataToList(fileName,ftpPath);
			...
			if(gsDataRecordList.size() > 0)
			{
				...
				gsSettlementMapper.delete(downloadDate,JOB_DIVIDER);
	...
	public boolean upload(int year, int month, int day)
	{
		...
		try
		{
			...
			gsRecordList = gsSettlementMapper.gssmSelect(uploadDate , otisOnline, pgOnline);
...
//				gsDataRecord.setGsPoint           (String.valueOf(gsSettlement.getGsPoint()));
			...
			logger.info("★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★");
			apacheFtpManager.connect(ftpServerIp , ftpId, password);
			apacheFtpManager.upload(ftpPath, filePath, fileName);
			logger.info("★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★");
